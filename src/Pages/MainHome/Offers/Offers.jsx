    ...
    return (
        <div className="pt-[80px] container font-secondry">
            ...
            <div className="flex items-center justify-between">
                <h1 className="text-dark text-[40px] tracking-wider font-bold">
                    {t("offers")}
                </h1>
                <Link
                    to="/offers"
                    className="bg-blue-600 text-white rounded-md px-6 py-2 hover:bg-blue-700 transition-all duration-300"
                >
                    {t("view_all")}
                </Link>
            </div>
                ...
                <div className="grid lg:grid-cols-3 xl:grid-cols-4 md:grid-cols-3 gap-4 mt-[30px] font-primary">
                    {products?.length > 0 ? (
                        products.map((product) => {
                            const wishlistIds =
                                product.product.whishlistes?.map((item) => item.product_id) || [];
                            const userIds =
                                product.product.whishlistes?.map((item) => item.user_id) || [];
                            const isInWishlist =
                                wishlistIds.includes(String(product.product.id)) &&
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex items-center justify-center relative">
                                        ...
                                        {product?.product?.images?.length > 0 && (
                                            <img
                                                ...
                                                alt={
                                                    product?.product?.translatable_name || "product image"
                                                }
                                            />
                                        )}
                                    </div>
                                    ...
                                    <div className="p-2 flex flex-col gap-1">
                                        ...
                                        >
                                            {product?.product?.translatable_name}
                                        </span>
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap">
                                            <div className="flex items-center gap-4">
                                                ...
                                                {product.new_price && (
                                                    <span className="font-semibold text-sm text-gray-600 line-through">
                                        ...
                                        <div className="flex items-center gap-2 mt-2">
                                            <div className="w-4 h-4 rounded-full bg-blue border border-slate-300"></div>
                                            <div className="w-4 h-4 rounded-full bg-red border border-slate-300"></div>
                                            <div className="w-4 h-4 rounded-full bg-gray border border-slate-300"></div>
                                        </div>
                                    </div>
                        ...
                        <div className="col-span-4 text-center py-10">
                            <h2 className="text-dark font-bold text-[20px]">
